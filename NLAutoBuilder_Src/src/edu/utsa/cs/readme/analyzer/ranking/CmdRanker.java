...
public class CmdRanker {
	...
	public static List<CmdRank> GetCmdsRankedByCmd(List<String> cmdtxt,String projpath)
	{
		...
		cmdprioritylist=config.GetProjectCmdPriorityList();
		...
		for(int i=0;i<cmdprioritylist.size();i++)
		{
			for(int j=0;j<rankedcmds.size();j++)
			{
				if((rankedcmds.get(j).getCmd().contains(cmdprioritylist.get(i))) && (FileFinder.IsConfigExists(projpath, cmdprioritylist.get(i))==true))
				{
					...
					if(rankedcmds.get(j).getRank()==0)
						rankedcmds.get(j).setRank(priority);
				}
